...
class APIController extends Controller
{
    ...
    function login(Request $request)
    {
        ...
        return ['code' => 200, 'status' => true, 'message' => 'Login Successfully', 'data' => $user, 'access_token' => $user->createToken($user->email)->plainTextToken];
    }
  ...
  //Register
    function register(Request $request)
    {
        ...
        Mail::send('mails.forgot-password', ['user' => $user, 'token' => $token], function ($m) use ($user, $token) {
            $m->from('info@codingsolution24.com', 'My CRM');
  ...
  //Update Profile
    function profile_update(Request $request)
    {
        ...
        if ($request->file()) {
            $imageName = rand(9, 99999999) . '.' . $request->image->extension();
            $request->image->move(public_path('uploads/user'), $imageName);
            $user->image = $imageName;
        }
        $user->save();
